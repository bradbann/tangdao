...
import org.tangdao.modules.paas.async.AsyncService;
...
@Service
public class MobileLocalServiceImpl implements IMobileLocalService {
    ...
    private AreaLocal pickupMobileLocal(String mobile) {
        ...
        try {
            ...
            AreaLocal pl = PaasRedisConstant.GLOBAL_MOBILES_LOCAL.get(area);
    ...
    private void load2Cache(List<AreaLocal> list) {
        for (AreaLocal pl : list) {
            PaasRedisConstant.GLOBAL_MOBILES_LOCAL.put(pl.getNumberArea(), pl);
        }
    }

    private String getKey() {
        return PaasRedisConstant.RED_AREA_MOBILES_LOCAL;
    }
    ...
    public boolean reload() {
        try {
            ...
            if (StringUtils.isNotEmpty(value)) {
                PaasRedisConstant.GLOBAL_MOBILES_LOCAL = JSON.parseObject(value,
            ...
            logger.info("Global mobiles local data[" + PaasRedisConstant.GLOBAL_MOBILES_LOCAL.size() + "] has loaded");
        }
