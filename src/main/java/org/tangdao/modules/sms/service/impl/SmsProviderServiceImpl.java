...
import org.tangdao.modules.exchanger.resolver.sms.cmpp.v3.Cmpp3ProxySender;
import org.tangdao.modules.exchanger.resolver.sms.http.SmsHttpSender;
import org.tangdao.modules.exchanger.resolver.sms.sgip.SgipProxySender;
...
import org.tangdao.modules.sms.mapper.SmsProviderMapper;
 ...
 */
@Service
public class SmsProviderServiceImpl extends CrudServiceImpl<SmsProviderMapper, SmsProvider> implements ISmsProviderService{
	@Autowired
    ...
    public List<ProviderSendResponse> sendSms(SmsPassageParameter parameter, String mobile, String content,
        ...
        String[] mobiles = mobile.split(MobileGroupUtils.DATA_SPLIT_CHARCATOR);
    ...
    public static List<String[]> recombineMobilesByLimitSpeedInSecond(String[] mobiles, Integer amount,
        ...
        for (int i = 0; i < groupSize; i++) {
            ...
            for (int j = 0; j < groupMobileSize && index < totalMobileSize; j++) {
                builder.append(mobiles[index++]).append(MobileGroupUtils.DATA_SPLIT_CHARCATOR);
                roundSize++;
            }
