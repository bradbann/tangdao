...
public class SmsSignatureExtnoServiceImpl extends CrudServiceImpl<SmsSignatureExtnoMapper, SmsSignatureExtno> implements ISmsSignatureExtnoService{
	...
	public boolean update(SmsSignatureExtno signatureExtNo) {
		SmsSignatureExtno td = get(signatureExtNo.getId());
		if(td == null) {
	...
	private String getFromDb(String userCode, String content) {
		List<SmsSignatureExtno> list = super.select(Wrappers.<SmsSignatureExtno>lambdaQuery().eq(SmsSignatureExtno::getUserCode, userCode));
		if(ListUtils.isEmpty(list)) {
            return null;
        }
	...
	public String getExtNumber(String userCode, String content) {
		if(StringUtils.isEmpty(content) || (!PatternUtils.isContainsSignature(content)) || StringUtils.isEmpty(userCode)) {
            return null;
        }
