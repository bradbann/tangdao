...
import org.tangdao.modules.sms.worker.fork.MtReportFailoverPushWorker;
import org.tangdao.modules.sms.worker.fork.MtReportPushToDeveloperWorker;
...
public class SmsMtMessagePushServiceImpl extends CrudServiceImpl<SmsMtMessagePushMapper, SmsMtMessagePush> implements ISmsMtPushService{
            ...
            for (SmsMtMessageSubmit submit : submits) {
                stringRedisTemplate.opsForHash().put(getMtPushConfigKey(submit.getMsgId()), submit.getMobile(),
                                                     JSON.toJSONString(submit,
                                                                       new SimplePropertyPreFilter("sid", "userCode",
                                                                                                   "msgId", "attach",
                                                                                                   "pushUrl",
                                                                                                   "retryTimes")));
    ...
    public SmsPushReport getPushReport(Map<String, Object> queryParams) {
        ...
        logger.info(JsonMapper.toJson(queryParams));
