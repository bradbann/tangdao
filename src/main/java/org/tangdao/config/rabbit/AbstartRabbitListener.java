package org.tangdao.config.rabbit;
...
import org.tangdao.config.runner.BootInitializeRunner;
...
public abstract class AbstartRabbitListener implements ChannelAwareMessageListener{
    ...
    protected void checkIsStartingConsumeMessage() {
        if (!BootInitializeRunner.isResourceInitFinished) {
            BootInitializeRunner.LOCK.lock();
            try {
                BootInitializeRunner.CONDITION.await();
            } catch (InterruptedException e) {
            } finally {
                BootInitializeRunner.LOCK.unlock();
            }
        }
    }
